...
def get_pose(image, model, device):
    ...
    image = letterbox(image, 256, stride=64, auto=True)[0]
...
def save_images_in_dir(frames, source):
    ...
    fall_images_dir = "./fall_images_dir"

    if not os.path.exists(fall_images_dir):
        os.makedirs(fall_images_dir)

    for i, frame in enumerate(frames, 1):
        img_name = f"{vid_name}_image_{i}.jpg"
        img_path = os.path.join(fall_images_dir, img_name)
...
"""
def processWebcam(vid_cap, device):
    frame_count = 0
    total_fps = 0
    time_list = []
    fps_list = []
    ...
    while True:
        ...
        for idx in range(output.shape[0]):
            plot_skeleton_kpts(_image, output[idx, 7:].T, 3)
        ...
        cv2.imshow("Pose Estimation Demo", _image)
...
Intended for display
"""
def processVideo(vid_cap, device):
    ...
    # fall_frames = List storing each of the video frames (Containing the fall)
    fall_frames = []
    ...
    save_images_in_dir(frames=fall_frames, source=source)
    avg_fps = total_fps / frame_count
    print(f"Average FPS = {avg_fps:.3f}")

    plot_fps_time_comparision(time_list, fps_list)


# Plot a graph showing how FPS varies over time
def plot_fps_time_comparision(time_list,fps_list):
    plt.figure()
    plt.xlabel('Time (in s)')
    plt.ylabel('FPS')
    plt.title('FPS and Frame Time Comparision Graph')
    plt.plot(time_list, fps_list,'b',label="FPS & Time")
    plt.savefig("FPS_and_Time_Comparision_pose_estimate.png")

def process(source, device):
    ...
    if source.isnumeric():    
        ...
        processVideo(vid_cap, device)

    print("Finished processing !!!")

def from_command_line():
    parser = argparse.ArgumentParser()
    parser.add_argument('--source', type=str, default='./Avi/fall_dataset/videos/video_7.mp4', help='video/0 for webcam') #video source
